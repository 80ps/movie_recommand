...
from implicit.als import AlternatingLeastSquares # 알고리즘(machin learning ) 사용을 위해 import
import pickle # 모델을 파일로 저장하기 위해 import
import sys
import json
...
def calculate_user_based(user_items, items):
  ...
  # recalculate_user=True : 사용자의 정보가 새롭게 주어졌을 때, 그 사용자의 특성을 다시 계산할지를 결정 / userid=0 이라 다시 계산함
  ...
  recs = loaded_model.recommend(userid=0, user_items=user_items, recalculate_user=True, N=3)  # N => 속도랑 관련될 수도 있음
  return [str(items[r]) for r in recs[0]] #추쳔 결과를 아이템 아이디에서 아이템 이름으로 변환하여 반환
...
if  __name__ =="__main__":
    ...
    if command == "item-based":
       ...
       print(json.dumps(result_items))
        ...
        print("Error: Invaild cammand error")
        sys.exit(1)


    # model_train()  # 데이터르 학습하여 모델을 생성하는 함수 호출, 처음 한번 실행 후 주석 처리, 이후 데이터가 추가됐을 때 주기적을  실행
